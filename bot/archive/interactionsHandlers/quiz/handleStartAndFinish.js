...
const { getTimeMinsAndSecs } = require("../../archive/utils/utilsFunctions");
const {
...
async function startQuizEmbed(client, interaction, quizData, questions) {
  ...
  const startButton = new ButtonBuilder().setCustomId("question_button_0").setLabel("Start Quiz").setStyle(ButtonStyle.Primary);
...
async function sendPersonalStatsEmbed(client, buttonInteraction, quizData, totalQuestions) {
  ...
  switch (quizData.visibility) {
    case 0: // No userdata will be published
      personalStats = await getPersonalStats(client, buttonInteraction.user.id, quizData.quiz_id, totalQuestions);
      break;
